...
<template>
  ...
  <div v-else-if="isAssistant" class="mx-auto dark:prose-invert prose lg:prose-lg prose-pre:p-0 flex flex-col group">
    ...
    <MessageTools v-if="!hasError && !isThinking && !!message.content && message.content.trim().length > 0"
      role="assistant" class="mt-2 opacity-0 group-hover:opacity-100 transition-opacity"
